...
def generate_report(date = "'2023-10-15'"):
    ...
    for r in rows:
        ...
        try:
            ...
            data_dict[r[0]]['Total Play Time (Minutes)'] = 0
...
st.title("liveDB :star2:")
st.subheader("You are connected to AD-IQ DailyLog LIVE Database!!")
st.caption("The database contains the Daily Log data for AD-IQ screens")

date_input = st.date_input("Enter Date:", format="YYYY-MM-DD")
# print(date)
# print(type(date))
...
generate_report_button = st.button("Generate Daily Report")

if generate_report_button:

    with st.spinner('Generating...'):

        st.title(f"Report for {date_input}")

        # Execute the query when the submit button is clicked
        df = generate_report(date)
